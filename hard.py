...
REVEALSPEED = 7  # 그림을 보여주는 속도(높을수록 빠름)
BOXSIZE = 60  # 그림 박스 크기
GAPSIZE = 10  # 그림 박스 간 간격
# 박스의 총 갯수 조절 가능 (easy:5,4 / normal:6,5 / hard:7,6)
BOARDWIDTH = 7  # 그림 박스 가로 크기
BOARDHEIGHT = 6  # 그림 박스 세로 크기
...
BGIMAGE = pygame.image.load("image/hard_racoon.png")
...
def main():
    ...
    # 게임 시작 시 음악 재생
    pygame.mixer.init()
    pygame.mixer.music.load("sound/hard.mp3")
    ...
    while (while_true):  # 게임 루프(Game Loof)
        ...
        message_Success = FONT.render("Success : {} / 21".format(Success), True, BLACK)
        message_Fail = FONT.render("Fail : {} / 20".format(Fail), True, BLACK)

        DISPLAY.blit(message_Success, (5, 0))
        DISPLAY.blit(message_Fail, (250, 0))
